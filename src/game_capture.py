...
class GameCapture:
    def __init__(self):
        ...
        self.color_ranges = {
            "player": {"lower": np.array([200, 0, 0]), "upper": np.array([255, 100, 100])},  # Red for player
            "enemy": {"lower": np.array([0, 0, 100]), "upper": np.array([100, 100, 255])},   # Blue for enemies
            "pickup": {"lower": np.array([200, 200, 0]), "upper": np.array([255, 255, 100])} # Yellow for pickups
        }
    ...
    def detect_health(self, frame: np.ndarray) -> int:
        ...
        self.logger.debug(f"Health detection: red pixels: {red_pixel_count}, estimated health: {estimated_health}")
    ...
    def detect_doors(self, frame: np.ndarray) -> Dict[str, bool]:
        ...
        door_regions = {
            "north": frame[int(height*0.05):int(height*0.15), int(width*0.45):int(width*0.55)],
            "east": frame[int(height*0.45):int(height*0.55), int(width*0.85):int(width*0.95)],
            "south": frame[int(height*0.85):int(height*0.95), int(width*0.45):int(width*0.55)],
            "west": frame[int(height*0.45):int(height*0.55), int(width*0.05):int(width*0.15)]
        }
    ...
    def save_debug_frame(self, frame: np.ndarray, game_state: Dict[str, Any], filename: str = "debug_frame.jpg") -> None:
        ...
        if game_state["player_position"]:
            ...
            cv2.putText(debug_frame, "Player", (x+10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        
        # Draw enemies
        for i, enemy in enumerate(game_state["enemies"]):
            x, y = enemy["position"]
            cv2.circle(debug_frame, (x, y), 10, (0, 0, 255), 2)
            cv2.putText(debug_frame, f"Enemy {i}", (x+5, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        
        # Draw items
        for i, item_pos in enumerate(game_state["items"]):
            x, y = item_pos
            cv2.circle(debug_frame, (x, y), 5, (255, 255, 0), 2)
            cv2.putText(debug_frame, f"Item {i}", (x+5, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
        ...
        # Draw door info
        door_text = "Doors: " + ", ".join([d for d, exists in game_state["doors"].items() if exists])
        cv2.putText(debug_frame, door_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
