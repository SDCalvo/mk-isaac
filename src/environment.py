try:
    ...
    from src.input_controller import InputController, Action
except ImportError:
...
class IsaacEnvironment:
    ...
    def step(self, action):
        ...
        if np.random.random() < 0.01:  # 1% chance each step
            self.capture.save_debug_frame(self.current_frame, game_state, f"debug_frame_{time.time()}.jpg")
        ...
        if current_position and self.previous_player_position:
            ...
            for direction, exists in current_doors.items():
                if exists:
                    ...
                    elif direction == "south" and y1 > y2 and y1 > self.current_frame.shape[0] * 0.8:
                        door_approached = True
