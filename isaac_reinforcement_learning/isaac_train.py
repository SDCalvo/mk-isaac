...
class DQNNetwork(nn.Module):
    ...
    def __init__(self, observation_space, action_space):
        ...
        self.input_size = (
            1 + 2 + 1 + 1 + 1  # health  # position (x, y)  # stage  # room_clear  # enemy_count
        )
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        states = {"health": [], "position": [], "stage": [], "room_clear": [], "enemy_count": []}
...
class DQNAgent:
    ...
    def select_action(self, state, training=True):
        ...
        """
        if training and (
            random.random() < self.epsilon or len(self.replay_buffer) < self.learning_starts
        ):
    ...
    def train(self, num_episodes, max_episode_steps=1000, save_interval=100, evaluation_episodes=5):
        ...
        for episode in range(1, num_episodes + 1):
            ...
            # Print statistics
            print(
                f"Episode {episode}/{num_episodes} - Reward: {episode_reward:.2f} - Loss: {avg_loss:.4f} - Steps: {episode_steps} - Epsilon: {self.epsilon:.4f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--episodes", type=int, default=1000, help="Number of episodes to train for"
    )
    parser.add_argument("--max-steps", type=int, default=2000, help="Maximum steps per episode")
    parser.add_argument(
        "--save-interval", type=int, default=50, help="Interval for saving the model"
    )
