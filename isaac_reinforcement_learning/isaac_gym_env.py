...
# Try to import isaac_communication first to avoid import-error
...
class IsaacGameResponseError(Exception):
    """Exception raised when there's an issue with game responses."""
...
class IsaacEnv(gym.Env):  # type: ignore
    ...
    def __init__(self, render_mode=None):
        ...
        try:
            # Check connection
            if self.client._check_connection():  # pylint: disable=protected-access
                self.connected = True
                print("Successfully connected to Isaac mod!")
            else:
                msg = "Failed to connect to Isaac mod. Make sure game is running with --luadebug."
                print(msg)
    ...
    def _get_obs(self):
        ...
        try:
            ...
            if response:
                ...
                # Using a specific exception for game communication issues
                raise IsaacGameResponseError("No response received from mod")
        except Exception as e:  # pylint: disable=broad-exception-caught
    ...
    def _calculate_reward(self, prev_state):
        ...
        if (
            prev_state["enemy_count"] > self.current_state["enemy_count"]
            and self.current_state["room_clear"] == 1
        ):
            reward += 1
            self.steps_since_last_room = 0
    ...
    def reset(self, seed=None, options=None):
        ...
        if not connected:
            msg1 = "WARNING: Could not connect to Isaac game. "
            msg2 = "Make sure the game is running with the mod installed."
            print(msg1 + msg2)
    ...
    def render(self):
        ...
        # The environment is automatically rendered by the game
...
def main():
    ...
    # Import argparse inside the function to avoid pylint issues
